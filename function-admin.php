...
function create_insta_post_type() {
    register_post_type('insta_post',
        array(
            ...
            'public'      => true,
            'has_archive' => true,
            'supports'    => array('title', 'editor'), // Asegurarse de que el soporte para thumbnails está habilitado
...
function render_insta_post_media_meta_box($post) {
    ...
    for ($i = 0; $i < 10; $i++) {
        ...
        echo '<button type="button" class="button select-media-button" data-target="#insta_post_media_'.$i.'">Select Media</button>';
        if ($value) {
